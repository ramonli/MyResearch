 ...
 * using the SLF4J framework instead. It also supports specifying a {@link Marker} instance that can be used by custom appenders and filters for the
 * bound logging toolkit to further process metrics reports.
 */
public class SmartSlf4jReporter extends ScheduledReporter {
	/**
	...
	/**
	 * A builder for {@link Slf4jReporter} instances. Defaults to logging to {@code metrics}, not using a marker, converting rates to events/second,
	...
	public static class Builder {
		 ...
		 *
		 * @param loggingLevel
		 *            a (@link Slf4jReporter.LoggingLevel}
		 * @return {@code this}
		 */
		 ...
		 */
		public SmartSlf4jReporter build() {
			...
			return new SmartSlf4jReporter(registry, loggerProxy, marker, prefix, rateUnit, durationUnit, filter, skipIdleMetrics);
		}
	}

	private static final Logger LOGGER = LoggerFactory.getLogger(SmartSlf4jReporter.class);
	...
	private SmartSlf4jReporter(MetricRegistry registry, LoggerProxy loggerProxy, Marker marker, String prefix, TimeUnit rateUnit, TimeUnit durationUnit, MetricFilter filter,boolean skipIdleMetrics) {
	...
	private void logTimer(String name, Timer timer) {
		...
		loggerProxy.log(marker,
		        "type=TIMER, name={}, count={}, min={}, max={}, mean={}, stddev={}, median={}, " + "p75={}, p95={}, p98={}, p99={}, p999={}, mean_rate={}, m1={}, m5={}, "
		                + "m15={}, rate_unit={}, duration_unit={}",
		        prefix(name), timer.getCount(), convertDuration(snapshot.getMin()), convertDuration(snapshot.getMax()), convertDuration(snapshot.getMean()),
		        convertDuration(snapshot.getStdDev()), convertDuration(snapshot.getMedian()), convertDuration(snapshot.get75thPercentile()),
		        convertDuration(snapshot.get95thPercentile()), convertDuration(snapshot.get98thPercentile()), convertDuration(snapshot.get99thPercentile()),
		        convertDuration(snapshot.get999thPercentile()), convertRate(timer.getMeanRate()), convertRate(timer.getOneMinuteRate()), convertRate(timer.getFiveMinuteRate()),
	...
	private void logMeter(String name, Meter meter) {
		...
		loggerProxy.log(marker, "type=METER, name={}, count={}, mean_rate={}, m1={}, m5={}, m15={}, rate_unit={}", prefix(name), meter.getCount(), convertRate(meter.getMeanRate()),
		        convertRate(meter.getOneMinuteRate()), convertRate(meter.getFiveMinuteRate()), convertRate(meter.getFifteenMinuteRate()), getRateUnit());
	}
	...
	private void logHistogram(String name, Histogram histogram) {
		...
		loggerProxy.log(marker, "type=HISTOGRAM, name={}, count={}, min={}, max={}, mean={}, stddev={}, " + "median={}, p75={}, p95={}, p98={}, p99={}, p999={}", prefix(name),
		        histogram.getCount(), snapshot.getMin(), snapshot.getMax(), snapshot.getMean(), snapshot.getStdDev(), snapshot.getMedian(), snapshot.get75thPercentile(),
		        snapshot.get95thPercentile(), snapshot.get98thPercentile(), snapshot.get99thPercentile(), snapshot.get999thPercentile());
	}
